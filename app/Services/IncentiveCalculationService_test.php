...
class IncentiveCalculationService
{
    ...
    $amountForDesignation = ($percentage / 100) * $baseAmount;
    if ($amountForDesignation <= 0) continue;
    ...
    // =====================================================
// 🔥 FIX 1: STORE-BASED POOLS (C&A, SM)
// =====================================================
if (in_array($desigName, $storeBasedPools)) {

    // ONLY employees assigned to this store
    ...
    foreach ($employees as $emp) {
        ...
        $newIncentive = EmployeeIncentive::create([
            ...
            'n_base_amount' => $baseAmount,
            'n_incentive_amount' => $amountPerEmployee,
            'n_pool_total_amount' => $amountForDesignation,
            'n_is_store_pool' => 1,
            'd_date' => $sale->d_date,
            'incentive_batch_id' => $batchId,
        ]);
    ...
    // =====================================================
    // 🔵 FIX 2: NORMAL POOL LOGIC (UNCHANGED)
    // =====================================================
    ...
    public function calculateStoreIncentives($storeId, $dateRange = null)
    {
        ...
        if ($dateRange && isset($dateRange['from']) && isset($dateRange['to'])) {
            $query->whereBetween('d_date', [$dateRange['from'], $dateRange['to']]);
        }
  ...
  foreach ($poolGroups as $poolName => $poolIncentives) {
    ...
    $isStorePool = in_array($poolNameKey, ['C&A', 'SM']);

    $percentage = $poolIncentives->first()->n_pool_percentage;

    $totalAmount = $isStorePool
        ? ($poolIncentives->first()->n_pool_total_amount ?? $poolIncentives->sum('n_incentive_amount'))
        : $poolIncentives->sum('n_incentive_amount');

    $employeeIncentives = $poolIncentives->groupBy('n_employee_id')->map(function ($items) {
        $employee = $items->first()->employee;

        return [
            'code' => $employee?->c_employee_code ?? 'N/A',
            'name' => $employee?->c_employee_name ?? 'Unknown',
