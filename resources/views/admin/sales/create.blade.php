        ...
        <form method="POST" id="frm_create" action="{{ route('admin.salesorders.store') }}">
            ...
            <div class="row">
                <div class="col-md-6 mb-3">
                    ...
                    <input type="date" name="d_date" class="form-control mandatory" data-message="Please Select a Date" value="{{ old('d_date', isset($sale) ? $sale->d_date->format('Y-m-d') : '') }}" {{isset($viewmode) && $viewmode=='on' ? 'readonly' : '' }}>
                    <div class="text-danger mt-1 fs-2"></div>
                </div>
                <div class="col-md-6 mb-3">
                    <label class="form-label">Bill No *</label>
                    <input type="text" name="c_bill_no" class="form-control mandatory" data-message="Please Enter Bill No" value="{{ old('c_bill_no',isset($sale) ? $sale->c_bill_no : '') }}" {{isset($viewmode) && $viewmode=='on' ? 'readonly' : '' }}>
                    <div class="text-danger mt-1 fs-2"></div>
                </div>
            ...
            <div class="row">
                <div class="col-md-6 mb-3">
                    ...
                    <select name="farm_care_advisor_id" data-message="Please Select Farm Care Advisor" class="form-select mandatory" {{isset($viewmode) && $viewmode=='on' ? 'disabled' : '' }}>
                        <option value="">Select</option>
                        @foreach($employees as $employee)
                        <option value="{{ $employee->n_employee_id }}" {{ old('farm_care_advisor_id', $sale->farm_care_advisor_id ?? '') == $employee->n_employee_id ? 'selected' : '' }} >
            ...
            <div class="row g-4 mb-4">
                <div class="col-md-6">
                    ...
                    <input type="text" id="c_customer_name" name="c_customer_name" value="{{ old('c_customer_name',isset($sale) ? $sale->c_customer_name : '') }}"
                       {{isset($viewmode) && $viewmode=='on' ? 'readonly' : '' }} data-message="Please add Customer Name" class="form-control mandatory" placeholder="Customer Name">
                    <div class="text-danger mt-1 fs-2"></div>
                </div>

                <div class="col-md-6">
                    <label for="c_customer_email" class="form-label">Customer Email *</label>
                    <input type="text" id="c_customer_email" name="c_customer_email" value="{{ old('c_customer_email',isset($sale) ? $sale->c_customer_email : '') }}"
                    ...
                    <select class="form-select mandatory" data-message="Please enter State" id="state" name="n_state_id" {{isset($viewmode) && $viewmode=='on' ? 'disabled' : '' }}>
                        ...
                        @if(isset($states))
                            @foreach($states as $State)
                                <option value="{{$State->n_state_id}}" {{ old('n_state_id', $sale->n_state_id ?? '') == $State->n_state_id ? 'selected' : '' }}>{{$State->name}}</option>
                            @endforeach
                        @endif
                    </select>
                    <div class="text-danger mt-1 fs-2"></div>
           ...
           <div class="row mb-3 align-items-center">
                ...
                <div class="col-md-9">
                    <div class="form-check form-check-inline">
                        <input class="form-check-input mandatory" type="radio" name="c_mode_of_payment" id="cod" data-message="Please enter Mode of Payment" value="cash_on_delivery" {{ old('c_mode_of_payment', $sale->c_mode_of_payment ?? '') == 'cash_on_delivery' ? 'checked' : '' }} {{isset($viewmode) && $viewmode=='on' ? 'disabled' : '' }}>
                    ...
                    <div class="form-check form-check-inline">
                        <input class="form-check-input" type="radio" name="c_mode_of_payment" id="upi" value="UPI" {{ old('c_mode_of_payment', $sale->c_mode_of_payment ?? '') == 'UPI' ? 'checked' : '' }} {{isset($viewmode) && $viewmode=='on' ? 'disabled' : '' }}>
             ...
             <div class="row g-4 mb-4">
                <div class="col-md-6">
                    ...
                    <select class="form-select mandatory" data-message="Please enter Nearest Franchise" id="franchise" name="nearest_franchise_id" {{isset($viewmode) && $viewmode=='on' ? 'disabled' : '' }}>
                        <option value="" selected>Select Franchise</option>
                         @if(isset($franchises))
                            @foreach($franchises as $franchise)
                                <option value="{{$franchise->n_store_id}}" {{ old('nearest_franchise_id', $sale->nearest_franchise_id ?? '') == $franchise->n_store_id ? 'selected' : '' }}>{{$franchise->c_store_name}}({{$franchise->c_store_code}})</option>
                            @endforeach
                        @endif
                ...
                <div class="col-md-6">
                    ...
                    <select class="form-select mandatory" data-message="Please enter Payment Status" id="payment_status" name="payment_status" {{isset($viewmode) && $viewmode=='on' ? 'disabled' : '' }}>
                        <option value="">Select Status</option>
                        <option value="pending" {{ old('payment_status', $sale->payment_status ?? '') == 'pending' ? 'selected' : '' }}>Pending</option>
                        <option value="approved" {{ old('payment_status', $sale->payment_status ?? '') == 'approved' ?  'selected' : '' }}>Approved</option>
            ...
            <div class="row g-4 mb-4">
                <div class="col-md-6">
                    ...
                    <select class="form-select mandatory" data-message="Please enter Delivory Status" id="delivery_status" name="delivery_status" {{isset($viewmode) && $viewmode=='on' ? 'disabled' : '' }}>
                        <option value="">Select Delivery Status</option>
                        <option value="pending" {{ old('delivery_status', $sale->delivery_status ?? '') == 'pending' ? 'selected' : '' }}>Pending</option>
                        <option value="shipped" {{ old('delivery_status', $sale->delivery_status ?? '') == 'shipped' ? 'selected' : '' }}>Shipped</option>
            ...
            <div class="mt-3">
                @if(isset($viewmode) && $viewmode=="on")
                    @can('sales-orders.approve')
                        <button type="button" style="width:150px;position:relative;" class="btn mb-1 btn-primary" data-bs-toggle="modal" data-bs-target="#approveModal" data-id="{{ isset($sale) ? Crypt::encryptString($sale->n_sl_no) : '' }}" id="approve">Approve</button>
                    @endcan
                @else
                    <button type="button" class="btn btn-primary" id="btn_create">{{isset($sale->n_sl_no) ? 'Update' : 'Create'}}</button>
