...
<style>
:root {
    --primary-green: #13deb9;
    --accent-orange: #13deb9;
    --soft-bg: #f8fafc;
    --border-color: #e2e8f0;
...
<div class="card draft-container mb-4">
    <div class="draft-header d-flex justify-content-between align-items-center">
        <h5 class="page-title mb-0">Draft Sales Preview</h5>
        @can('bulk-upload.upload')
        <a href="{{ route('admin.sales.bulk-upload') }}" class="btn btn-outline-secondary btn-sm fw-bold">
            <i class="ti ti-upload me-1"></i> New Upload
        </a>
        @endcan
    </div>
    ...
    <div class="card-body p-4">
        ...
        <div class="metric-bar mb-4">
            ...
            <div class="ms-auto d-flex align-items-center gap-2">
                ...
                <select name="per_page" id="per_page" class="form-select form-select-sm border-0 bg-light"
                    style="width: 80px;"
                    onchange="window.location.href = '{{ route('admin.sales.drafts') }}?per_page=' + this.value">
        ...
        <div class="action-bar mb-4">
            <div class="d-flex gap-2">
                @can('draft-sales.confirm')
                @if($counts['valid'] > 0)
                <form action="{{ route('admin.sales.confirm-drafts') }}" method="POST"
                    onsubmit="return confirm('Import {{ $counts['valid'] }} valid sale(s) to the main table?')">
                    @csrf
                    <button type="submit" class="btn btn-confirm">
                        <i class="ti ti-check-double me-1"></i> Confirm {{ $counts['valid'] }} Valid Sales
                    </button>
                </form>
                @endif
                @endcan
                @can('draft-sales.cancel')
                <form action="{{ route('admin.sales.cancel-drafts') }}" method="POST"
                    onsubmit="return confirm('Cancel and delete all drafts?')">
                    @csrf
            ...
            <table class="table table-custom align-middle text-nowrap">
                ...
                <tbody>
                    ...
                    <tr class="{{ $rowHighlight }}">
                        ...
                        <td>
                            @if($draft->c_billno)
                            <a href="{{ route('admin.sales.draft-bill', $draft->c_billno) }}"
                    ...
                    <tr>
                        <td colspan="10" class="text-center py-5 text-muted">
                            ...
                            No draft records found. Upload a file to begin.
                        </td>
                    </tr>
                    @endforelse
                </tbody>
